...
from modules.map.proto.map_road_pb2 import Road
...
class MapParser:
    ...
    __is_lane_in_junction: dict
    __lanes_controlled_by_signal: dict # key:signal_id
    __lane_controlled_by_signals: dict # key:lane_id
    ...
    def __init__(self, filename: str) -> None:
        ...
        Constructor
        """
        self.__map = hdmap.load_hd_map(filename)
        self.load_junctions()
        self.load_signals()
    ...
    def load_roads(self):
        self.__roads = dict()
        for r in self.__map.road:
            self.__roads[r.id.id] = r
    ...
    def parse_relations(self):
        ...
        self.__is_lane_in_junction = dict()
        for lank, lanv in self.__lanes.items():
            self.__is_lane_in_junction[lank] = False
            for junk, junv in self.__junctions.items():
                if self.__is_overlap(lanv, junv):
                    self.__lanes_at_junction[junk].append(lank)
                    self.__is_lane_in_junction[lank] = junk

        # load lanes_controlled_by_signal and lane_controlled_by_signals
        self.__lanes_controlled_by_signal = defaultdict(lambda: list())
        self.__lane_controlled_by_signals = defaultdict(lambda: list())
        for junk, junv in self.__junctions.items():
            ...
            for sid in signal_ids:
                for lid in lane_ids:
                    if self.__is_overlap(self.__signals[sid], self.__lanes[lid]):
                        ...
                        self.__lane_controlled_by_signals[lid].append(sid)
    ...
    def get_roads(self) -> List[str]:
        return list(self.__roads.keys())
    
    def get_road_by_id(self, r_id:str) -> Road:
        return self.__roads[r_id]
    ...
    def get_is_lane_in_junction(self):
        return self.__is_lane_in_junction
    
    def get_lane_controlled_by_signals(self, lane_id):
        return self.__lane_controlled_by_signals[lane_id]

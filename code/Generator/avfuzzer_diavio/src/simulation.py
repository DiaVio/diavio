...
class LgApSimulation:
    ...
    def __init__(self):
        ################################################################
        self.totalSimTime = config.totalSimTime
        ...
        self.initEvPos = config.initEvPos
        ...
        # Add NPCs: Hard code for now, the number of npc need to be consistent.
        ################################################################
        self.addNpcVehicle(lgsvl.Vector(-50, -1, -20))
        self.addNpcVehicle(lgsvl.Vector(-100, -1, -9))
        #self.addNpcVehicle(lgsvl.Vector(164, 88.38, -608.9))
        ################################################################
        self.isEgoFault = False
        self.isHit = False
        sd.init_global(self.sim, self.ego, self.npcList)
        self.connectEvToApollo()
        self.maxint = 130
        self.egoFaultDeltaD = 0

    def initSimulator(self):
        sim = lgsvl.Simulator(config.SIMULATOR_HOST, config.SIMULATOR_PORT) 
        self.sim = sim
    ...
    def initEV(self):
        ...
        #egoState.transform.rotation.y = 270
        #ego = sim.add_agent("XE_Rigged-apollo_3_5", lgsvl.AgentType.EGO, egoState)
        #
        ego = sim.add_agent(config.vehicle_conf, lgsvl.AgentType.EGO, egoState)
        #
    ...
    def connectEvToApollo(self):
        ...
        ego.connect_bridge(config.BRIDGE_HOST, config.BRIDGE_PORT)
        while not ego.bridge_connected:
            time.sleep(1)
            print("in loop")
        dv = lgsvl.dreamview.Connection(self.sim, ego, config.BRIDGE_HOST)
        print("after dreamview.connection")
        dv.set_hd_map(config.LGSVL__AUTOPILOT_HD_MAP)
        dv.set_vehicle(config.LGSVL__AUTOPILOT_0_VEHICLE_CONFIG)
        #dv.disable_apollo()
        dv.setup_apollo(config.destination['x'], config.destination['z'],['Localization','Transform','Routing','Prediction','Planning','Control','Recorder'])#,
    ...
    def runSimulation(self, scenarioObj):
        ...
        def on_collision(agent1, agent2, contact):
            ...
            sd.add_collision(agent1,agent2)
        ...
        for t in range(0, int(numOfTimeSlice)):
            ...
            for j in range(0, int(actionChangeFreq) * 4):
                ...
                while not ego.bridge_connected or "fail" in line or "Fail" in line or "overflow" in line:
                    ...
                    return resultDic
                '''
                sim.run(config.timeSliceLength)
                sd._simHistory.add(ego, npcList)
        ...
        resultDic['fault'] = ''
        if self.isHit == True:
            resultDic['fault'] = 'unknown'
        if self.isEgoFault == True:
            resultDic['fault'] = 'ego'
        util.print_debug(" === Finish simulation === ")
        util.print_debug(resultDic)
        sd.dump_report(self.isHit)
        #sd._simHistory.speed_print(ego)
